...
export default function Groups({ groups }) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-8 p-6 z-20">
            {groups?.length > 0 && groups?.map((group) => (
                <div key={group.population} className=" bg-slate-300 text-black rounded-lg text-center items-center flex flex-col">
                    <div className="p-4">{group.Ctry}</div>
                        ...
                        <div className="px-4 pb-2" key={group.population}>Unreached: {convertToPercentage(group.PoplPeoplesLR / group.Population)}</div>
                </div>
            ))}
        </div>
    )
