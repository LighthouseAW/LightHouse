        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-4 p-6 mt-20">
            {projects.map((project, index) => {
                ...
                return (
                    ...
                    <Link href={`/projects/${project.id}`} className=" underline font-semibold text-black text-lg mb-2 text-center">{project.name}</Link>
                    ...
                    <Link href={`/projects/${project.id}`} className=" font-semibold text-black text-lg text-center">Learn More →</Link>
                </div>
            )})}
        </div>
    )
